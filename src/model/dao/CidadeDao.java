...
public class CidadeDao extends Dao implements DaoI<Cidade> {
   ...
   String sql = "SELECT IDCIDADE, NOME, UF FROM CIDADE WHERE ATIVO = 1 AND NOME LIKE '"+pesquisa+"%' ORDER BY NOME";
        try {
    ...
    public List<Cidade> pesquisarTodos(String orderby, String ordenacao) {
        String sql = "SELECT IDCIDADE, NOME, UF FROM CIDADE WHERE ATIVO = 1 ORDER BY " + orderby + " " + ordenacao;
        try {
            PreparedStatement stmt = conexao.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
            List<Cidade> listCidade = new ArrayList<>();
